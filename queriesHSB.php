...
function get_wc_export_query_hsb($the_prefix, $prod_id, $from_date, $to_date, $cost){
    return "SELECT
    u1.id,
    COALESCE(
        TRIM(
...
function get_wc_export_query1($the_prefix, $prod_id, $from_date, $to_date, $cost1){
    return "SELECT
    u1.id,
    COALESCE(
        TRIM(
...
WHERE
    ...
    AND DATE(pm2_fil.paid_date) >= STR_TO_DATE('{$from_date}', '%Y-%m-%d')
    AND DATE(pm2_fil.paid_date) <= STR_TO_DATE('{$to_date}', '%Y-%m-%d')
    AND pm12._order_total <> {$cost1}    
ORDER BY
    pm2_fil.paid_date ASC";
}

function get_wc_export_query2($the_prefix, $prod_id, $from_date, $to_date, $cost1, $cost2){
    return "SELECT
    u1.id,
    COALESCE(
        TRIM(
